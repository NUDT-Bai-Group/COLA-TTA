                    ...
                    do
                        ...
                        do
                            ...
                            do
                                ...
                                model_src.arch="${model}" model_src.backbone="${backbone}" 2>&1 | tee "VISDA-C_${model}_$(echo ${backbone} | sed 's|/|_|g')-Ratio(1-1-1)-hidden-${hidden_size}_LR-${LR}_MLP-${mlp_lr}_Thres(${probs_thres}-${num_thres})_Target_{${target_domains}}_$(date '+%Y-%m-%d_%H_%M_%S').txt" 2>&1
                            done
                        done
                    done
                done
